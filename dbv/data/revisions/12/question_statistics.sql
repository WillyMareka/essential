...
WHEN 'national' THEN 
SELECT 
    COUNT(DISTINCT wp.ss_id) AS total,
...
SELECT 
     q.question_code,
    count(DISTINCT(lq.hcw_id)) as total_response,
    lq_response as response,
    (CASE WHEN (lq.lq_response = '' || lq.lq_response = 'n/a' 
    || lq.lq_response IS NULL) THEN 'No data' 
            WHEN lq.lq_response = 'Yes' THEN 'Yes' 
...
GROUP BY lq.question_code, response
ORDER BY lq.question_code,count(DISTINCT lq.hcw_id);
...
WHEN 'national' THEN
SELECT 
	hl.names_of_participant as Health_Care_Worker,
    hl.mobile_number as Phone_Number,
    hl.id as Personal_Number,
    hl.id_number as National_ID,
    sp.spoint as Current_Service_Unit,
    hl.training_location as County_Original_Facility_Trained_in,

    (CASE (q.question_code = 'QUC32')
        WHEN lq.lq_response = 'Yes' THEN 'Yes'
        ELSE 'No'
    END) AS Still_Working_at_Original_Facility_where_trained,

    (CASE (q.question_code = 'QUC33')
        WHEN lq.lq_response = 'Yes' THEN 'Yes'
        ELSE 'No'
    END) AS Transferred_within_Same_County,

    (CASE (q.question_code = 'QUC34')
        WHEN lq.lq_response = 'Yes' THEN 'Yes'
        ELSE 'No'
    END) AS Transferred_to_Another_County,

    q.question_name as Certification,
    NULL as Assessment_Outcome
FROM
    log_questions_hcw lq
        JOIN
            questions q ON (lq.question_code = q.question_code
            AND q.question_for = questionfor)
        JOIN 
            hcw_list hl ON hl.id = lq.hcw_id
        JOIN 
	work_profile wp ON ss_id = lq.hcw_id 
	JOIN 
            service_point sp ON sp.spoint_code = wp.lq_response
...
SELECT 
    COUNT(DISTINCT f.fac_mfl) AS total_response,
    lq.lq_specified_or_follow_up AS response,
    q.question_name,
    q.question_code,f.fac_tier
FROM
    log_questions lq
        JOIN
    questions q ON lq.question_code = q.question_code
    ...
    survey_categories sc ON ss.sc_id = sc.sc_id
        AND sc.sc_name = survey_category
        WHERE q.question_code = 'QMNH02'
GROUP BY lq.lq_specified_or_follow_up , q.question_name,f.fac_tier;
