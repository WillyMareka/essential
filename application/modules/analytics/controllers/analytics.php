...
class Analytics extends MY_Controller
{
    ...
    */
    public function getTrainedStaff($criteria, $value, $survey, $survey_category, $for) {
        ...
        $results = $this->analytics_model->getTrainedStaff($criteria, $value, $survey, $survey_category, $for,'total');
        ...
        $this->populateGraph($resultArray, '', $category, $criteria, 'normal', 90, 'bar','',$for, 'staff_trained', 'total', $colors);
    ...
    public function getStaffRetention($criteria, $value, $survey, $survey_category, $for) {
        ...
        $results = $this->analytics_model->getStaffRetention($criteria, $value, $survey, $survey_category, $for, 'total');
    ...
    public function getTrainedStaffRAW($criteria, $value, $survey, $survey_category, $for,$form) {
        ...
        $results = $this->arrays->reset($results);
        // echo '<pre>';print_r($results);die;
        echo $this->export->generate($results, 'Staff Trained Statistics for' . ucwords($for) . '(' . $value . ')', $form);
    }

    public function getStaffAvailabilityCH($criteria, $value, $survey, $survey_category){
           $this->getStaffAvailability($criteria, $value, $survey, $survey_category, 'ch');
    }
    ...
    public function getStaffAvailability($criteria, $value, $survey, $survey_category, $for) {
        ...
        $results = $this->analytics_model->getStaffAvailability($criteria, $value, $survey, $survey_category, $for,'total');
        //echo "<pre>";print_r($results);echo "</pre>";die;
        $category = array();
        ...
        //echo "<pre>";print_r($resultArray);echo "</pre>";die;
        $this->populateGraph($resultArray, '', $category, $criteria, 'normal', 90, 'bar', '', $for, 'staff_availability', 'total', $colors);
    }
    ...
    public function getTreatmentStatistics($criteria, $value, $survey, $survey_category, $statistic, $option) {
        ...
        if ($statistic == 'treatment' || $statistic == 'other_treatment') {
            ...
            $resultArray = array(array('name' => 'Diarrhoea', 'data' => array($gData['dia'], 0, 0, 0)), array('name' => 'Malaria', 'data' => array(0, $gData['fev'], 0, 0)), array('name' => 'Pneumonia', 'data' => array(0, 0, $gData['pne'], 0)), array('name' => 'Other Totals', 'data' => array(0, 0, 0, $gData[0])));
    ...
    public function getSuppliesStatistics($criteria, $value, $survey, $survey_category, $for, $statistic) {
        ...
        if (($statistic == 'location' && $for == 'mh')) {
        ...
        } else if (($statistic == 'availability' && $for == 'ch') || ($statistic == 'availability' && $for == 'mnh') || ($statistic == 'availability' && $for == 'tst') || ($statistic == 'unavailability' && $for == 'mnh')) {
            ...
            foreach ($results as $key => $result) {
                ...
                $category[] = $key;
                foreach ($result as $name => $value) {
                    if ($name != 'Sometimes Available' && $name != '') {
            ...
            $colors = array('#2f7ed8', '#0d233a', '#8bbc21', '#910000', '#1aadce', '#492970', '#f28f43', '#77a1e5', '#c42525', '#dddddd');
            $this->populateGraph($resultArray, '', $category, $criteria, 'percent', 130, 'bar', (int)sizeof($category), $for, $statistic, $colors);
            ...
            $this->populateGraph($resultArray, '', $category, $criteria, 'percent', 130, 'column', (int)sizeof($category), $for, 'supplies', $statistic, $colors);
        }
 ...
 $results = $this->analytics_model->getSuppliesStatistics($criteria, $value, $survey, $survey_category,$for,$statistic);
     ...
     * @return [type]           [description]
     */
    public function getMNHSuppliesReason($criteria, $value, $survey, $survey_category) {
        $this->getSuppliesStatistics($criteria, $value, $survey, $survey_category, 'mnh', 'unavailability');
    }
    ...
    */
    ...
    public function getMNHDeliveryKitsLocation($criteria, $value, $survey, $survey_category) {
        $this->getEquipmentStatistics($criteria, $value, $survey, $survey_category, 'dke', 'location');
    }
       ...
       //echo "<pre>";print_r($results);echo "</pre>";die;
        ...
        foreach ($data as $k => $val) {
            ...
            foreach ($gdata as $name => $value1) {
                if ($name == 'No data') {
                    $color = '#dddddd';
                } elseif (($name == 'Available') || ($name == 'functional')) {
                    $color = '#8bbc21';
                } elseif (($name == 'Not Available') || ($name == 'nonfunctional')) {
                    $color = '#fb4347';
                } else {
                    $color = $colors[$colorCounter];
                    $colorCounter++;
        ...
        } else if ($statistics == 'hcwRetention' && $for == 'wp') {
            $results = $this->analytics_model->getQuestionStatistics($criteria, $value, '', '', $for, $statistics);
            $number = $resultArray = $q = $data = $gdata = $res = array();
            $number = $resultArray = $q = $yes = $no = $null = array();
            $colors = array('#2f7ed8', '#0d233a', '#8bbc21', '#910000', '#1aadce', '#492970', '#f28f43', '#77a1e5', '#c42525', '#dddddd');
            ...
            foreach ($data as $k => $val) {
                foreach ($val as $name => $value_) {
                  //echo '<pre>'; print_r($r); echo '</pre>';die;
                    ...
                    $gData[] = array('name' => $name, 'y' => (int)$value_, 'color'=> $color);
                }
            }
            // $colors = array('#2f7ed8', '#0d233a', '#8bbc21', '#910000', '#1aadce', '#492970', '#f28f43', '#77a1e5', '#c42525', '#dddddd');
            // $colorCounter = 0;
            ...
            $this->populateGraph($resultArray, '', $category, $criteria, 'percent', 90, 'pie', '', $for, 'question', $statistics, $colors);
        } else if ($statistics == 'hcwTransfer' && $for == 'wp') {
            $results = $this->analytics_model->getQuestionStatistics($criteria, $value, '', '', $for, $statistics);
            $number = $resultArray = $q = $data = $gdata = $res = array();
            $number = $resultArray = $q = $yes = $no = $null = array();
            foreach ($results as $key => $value) {
           ...
           $this->populateGraph($resultArray, '', $category, $criteria, 'percent', 90, 'bar', '', $for, 'question', $statistics, $colors);
            ...
            $resultArray[]=array('name' => 'Skilled Birth Attendants','data' => $gData);
            //echo "<pre>";print_r($resultArray);echo "</pre>";die;
            $this->populateGraph($resultArray, '', $category, $criteria, '', 90, 'bar', '', $for, 'question', $statistics, $colors);
        }else {
            $number = $resultArray = $q = $data = $gdata = $res = array();
            $number = $resultArray = $q = $yes = $no = $null = array();
            foreach ($results as $key => $value) {
     ...
     * @return [type]                  [description]
     */
    public function getHCWProfileRaw($criteria, $value, $survey, $survey_category, $statistics, $form) {
    ...
    public function getIndicatorAssessmentRaw($criteria, $value, $survey, $survey_category, $for, $statistics, $form) {
        $results = $this->analytics_model->getIndicatorComparison($criteria, $value, $survey, $survey_category, $for, 'hcwassessment_raw');
    ...
    public function getDiarrhoeaStatistics($criteria, $value, $survey, $survey_category,$statistic) {
        $results = $this->analytics_model->getDiarrhoeaStatistics($criteria, $value, $survey, $survey_category,'response');
     ...
     public function getDiarrhoeaRaw($criteria, $value, $survey, $survey_category,$statistic, $form) {
          ...
          //echo "<pre>";print_r($results);echo "</pre>";die;
        $results = $this->arrays->reset($results);
      echo $this->export->generate($results, 'Diarrhoea Statistics for' . ucwords($for) . '(' . $value . ')', $form);
    ...
    public function getTreatmentCounseling($criteria, $value, $survey, $survey_category) {
        $this->getQuestionStatistics($criteria, $value, '', '', 'cnl', 'hcwresponse');
    }
    ...
    public function getIndicatorComparison($criteria, $value, $survey, $survey_category, $for, $statistic) {
        ...
        if ($statistic = 'classification' && $for = 'ch') {
            ...
            //echo '<pre>';print_r($resultArray);echo '</pre>';die;
            $this->populateGraph($resultArray, '', $category, $criteria, 'percent', 130, 'bar', '', $for, 'classification', $statistics, $color);
    ...
    public function getAssessmentComparison($criteria, $value, $survey, $survey_category, $for, $statistic) {
        ...
        foreach ($results as $indicator => $values) {
            $category[] = $indicator;
            //echo '<pre>';print_r($indicator);echo '</pre>';die;
            foreach ($values as $verdict => $answer) {
                $gData[$verdict][] = $answer;
            }
        }
    ...
    public function getIndicatorTypes2A() {
        ...
        foreach ($results as $value) {
            if (($value['il_for'] == 'pne') || ($value['il_for'] == 'dgn') || ($value['il_for'] == 'fev') || ($value['il_for'] == 'ear')) {
                if (($value['il_for']) == 'pne') {
                    $value['il_full_name'] = 'Cough / Difficulty in Breathing';
                }
                $options.= '<option value="' . $value['il_for'] . '">' . $value['il_full_name'] . '</option>';
            }
        }
    ...
    public function getIndicatorTypes2B() {
        ...
        $options = '<option>Select Main Symptom/Condition</option>';
        foreach ($results as $value) {
            if (($value['il_for'] == 'svd') || ($value['il_for'] == 'jau') || ($value['il_for'] == 'eye') || ($value['il_for'] == 'stn') || ($value['il_for'] == 'dgn') || ($value['il_for'] == 'fed')) {
              if (($value['il_for']) == 'svd') {
                    $value['il_full_name'] = 'Very Severe Disease';
              }elseif (($value['il_for']) == 'fed') {
                    $value['il_full_name'] = 'Feeding Problem';
              }elseif (($value['il_for']) == 'stn') {
                    $value['il_full_name'] = 'Special Treatment Needs';
                }
                $options.= '<option value="' . $value['il_for'] . '">' . $value['il_full_name'] . '</option>';
            }
        }
    ...
    public function getIndicatorTypes3() {
        $results = $this->analytics_model->getIndicatorTypes3();
        ...
        foreach ($results as $value) {
          ...
          foreach ($value as $name => $con) {
             //echo '<pre>';print_r($con);echo '</pre>';die;
            
                
                $options.= '<option value="' . $con . '">' . $con . '</option>';
 ...
 $results = $this->analytics_model->getBemONCReason($criteria, $value, $survey, $survey_category,'response_raw');
    ...
    public function getCommunityStrategyMNH($criteria, $value, $survey, $survey_category, $option) {
        ...
        $resultArray[] = array('name' => 'Numbers', 'data' => $gData);
        $this->populateGraph($resultArray, '', $category, $criteria, '', 70, 'bar', '', 'cms', 'question', 'total');
    }
